...
- 800 ms debounce per field, with in-flight requests aborted when you keep typing.
...
- **LanguageTool public API**: rate-limited to roughly 20 requests per minute per IP and ~20 KB per text. The 800 ms debounce keeps normal typing well under that, and we truncate text to 18 KB before sending. Heavy editing on multiple tabs at once may hit the cap; in that case the next check will silently fail and surface as a `[grammar-pal] check failed` warning in the page console.
